...
####################################################################
# Classification Ertel et al., Cell Cycle 2010, 159 genes 'RB-loss'
####################################################################
`RBloss` <- function(DATA, ID, ER=NULL){
# DATA, data matrix (log2-transformed)
# ID, vector of Symbol genes
# ER, vector of samples estrogen receptor statut (bin, 1|0)
#
	#### Matching DATA w/ model
	intsectSymbol <- setdiff(intersect(ID, .rbloss_model$ref$Symbol), which(is.na(.rbloss_model$ref$Symbol)))
	if (length(intsectSymbol)==0){ stop("\nno match\n") }
	dat <- DATA[match(intsectSymbol, ID),]
	ref <- .rbloss_model$ref[match(intsectSymbol, .rbloss_model$ref$Symbol),]
	# adjust data by median center genes on ER+ samples
	if (is.null(ER)){stop("\nER status required\n")}
	selERpos <- which(ER %in% c(1,"1"))
	if (length(selERpos)<2) {stop("\nER+ samples missing\n")}
	dat <- (dat - apply(dat[,selERpos],1,function(x){mean(x,na.rm=T)}))/apply(dat[,selERpos],1,sd,na.rm=T)
	# Metagene
	Score_mat <- list(score=apply(dat,2,median,na.rm=T))
	Score_mat$grp <- cut(Score_mat$score, c(-Inf, 0.2499, 0.7299, Inf), c("Low","Intermed","High"))
	Score_mat$grp2K <- cut(Score_mat$score, c(-Inf, 0.7299, Inf), c("Low","High"))
	# output
	return(data.frame(Score_mat))
}
#####################################################################
